marginal_gof_scr = function(fit_0_par, fit_1_par, 
                            ...
                            data, cop_type,
                            grid){
  nknots = length(knots0) - 2
  par(mfrow = c(2,2))
  #goodness of fit of marginal survival function of OS
  Surv_t0 = 1 - psurvspline(q = grid, gamma = fit_0_par[(nknots + 3):(2*(nknots + 2))], 
                            knots = knott0)
  ...
  Surv_t1 = 1 - psurvspline(q = grid, gamma = fit_1_par[(nknots + 3):(2*(nknots + 2))], 
                            knots = knott1)
  ...
  probs0 = sapply(grid, pfs_surv, gammas = fit_0_par[1:(nknots + 2)], 
                  gammat = fit_0_par[(nknots + 3):(2*(nknots + 2))],
                  knots = knots0, knott = knott0, theta = fit_0_par[2*(nknots + 2) + 1])
  ...
  probs1 = sapply(grid, pfs_surv, gammas = fit_1_par[1:(nknots + 2)], 
                  gammat = fit_1_par[(nknots + 3):(2*(nknots + 2))],
                  knots = knots1, knott = knott1, theta = fit_1_par[2*(nknots + 2) + 1])
  plot(survfit(Surv(data$Pfs, pmax(data$PfsInd, data$SurvInd))~1, data = data, 
               subset = data$Treat == 1),
...
marginal_gof_no = function(fit_0_par, fit_1_par, 
  ...
  Surv_t1 = 1 - psurvspline(q = grid, gamma = fit_1_par[(nknots + 3):(2*(nknots + 2))], knots = knott1)
  plot(survfit(Surv(Surv, SurvInd)~1, data = data, subset = data$Treat == 1),
       xlim = c(min(grid), max(grid)), main = "OS (1)", xlab = "time (months)", ylab = "S(t)")
  lines(grid, Surv_t1, col = "red")
  
  probs0 = 1 - psurvspline(q = grid, gamma = fit_0_par[1:(nknots + 2)], knots = knots0)
  plot(survfit(Surv(data$Pfs, pmax(data_pfs$PfsInd, data_pfs$SurvInd))~1, data = data_pfs, 
               subset = data_pfs$Treat == 0),
       xlim = c(min(grid), max(grid)), main = "PFS (0)", xlab = "time (months)", ylab = "S(t)")
  lines(grid, probs0, col = "red")
  
  probs1 = 1 - psurvspline(q = grid, gamma = fit_1_par[1:(nknots + 2)], knots = knots1)
